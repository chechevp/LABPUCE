...
namespace LaboratorioPUCE.Controllers
{
    ...
    public class EstudianteController : ControllerBase
    {
        ...
        public async Task<IActionResult> SolicitarDevolucion(string codigoReserva)
        {
            var authHeader = Request.Headers["Authorization"].ToString();
            if (string.IsNullOrEmpty(authHeader) || !authHeader.StartsWith("Bearer "))
                return Unauthorized(new { mensaje = "Token ausente." });
            ...
            var exito = await _prestamosService.SolicitarDevolucionAsync(codigoReserva, session.UsuarioId);
